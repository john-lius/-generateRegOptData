...
import com.digihealth.basedata.sql.BasObserveDataSql;
import com.digihealth.utils.ConnectionManager;

public class BasObserveDataDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(BasObserveDataSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------BasObserveDataDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
