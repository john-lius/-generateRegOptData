...
import com.digihealth.doc.sql.DocInsuredItemSql;
import com.digihealth.utils.ConnectionManager;

public class DocInsuredItemDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(DocInsuredItemSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocInsuredItemDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
