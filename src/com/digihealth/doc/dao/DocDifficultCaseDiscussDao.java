...
import com.digihealth.doc.sql.DocDifficultCaseDiscussSql;
import com.digihealth.utils.ConnectionManager;

public class DocDifficultCaseDiscussDao {

	public void insert(DocDifficultCaseDiscuss entity) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocDifficultCaseDiscussSql.insert);
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocDifficultCaseDiscussSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocDifficultCaseDiscussDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
