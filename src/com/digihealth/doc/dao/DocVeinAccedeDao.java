...
import com.digihealth.doc.sql.DocVeinAccedeSql;
import com.digihealth.utils.ConnectionManager;

public class DocVeinAccedeDao {

	public void insert(DocVeinAccede entity) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocVeinAccedeSql.insert);
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocVeinAccedeSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocVeinAccedeDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
