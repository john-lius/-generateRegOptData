...
import com.digihealth.doc.sql.DocPatPrevisitRecordSql;
import com.digihealth.utils.ConnectionManager;

public class DocPatPrevisitRecordDao {

	public void insert(DocPatPrevisitRecord entity) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocPatPrevisitRecordSql.insert);
			pstmt.setString(1, entity.getPatVisitId());
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocPatPrevisitRecordSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocPatPrevisitRecordDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
