...
public class DocAnaesSummaryAppendixGenDao {
	...
	public void insert(DocAnaesSummaryAppendixGen entity) {
		...
		try {
			...
			System.out.println("执行insert语句出现异常(DocAnaesSummaryAppendixGenDao)："
					+ e.getMessage());
		} finally {
			try {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocAnaesSummaryAppendixGenSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocAnaesSummaryAppendixGenDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
