...
import com.digihealth.doc.sql.DocSelfPayAccedeLlzyySql;
import com.digihealth.utils.ConnectionManager;

public class DocSelfPayAccedeLlzyyDao {

	public void insert(DocSelfPayAccedeLlzyy entity) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocSelfPayAccedeLlzyySql.insert);
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocSelfPayAccedeLlzyySql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocSelfPayAccedeLlzyyDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
