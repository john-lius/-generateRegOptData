...
import com.digihealth.doc.sql.DocSelfPayInstrumentAccedeSql;
import com.digihealth.utils.ConnectionManager;

public class DocSelfPayInstrumentAccedeDao {

	public void insert(DocSelfPayInstrumentAccede entity) {
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocSelfPayInstrumentAccedeSql.insert);
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocSelfPayInstrumentAccedeSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocSelfPayInstrumentAccedeDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
