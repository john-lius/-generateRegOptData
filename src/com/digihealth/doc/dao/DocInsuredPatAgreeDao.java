...
public class DocInsuredPatAgreeDao {
	...
	public void insert(DocInsuredPatAgree entity) {
		...
		try {
			...
			System.out.println("执行insert语句出现异常(DocInsuredPatAgreeDao)："
					+ e.getMessage());
		} finally {
			try {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			conn = ConnectionManager.getAISDEVConnection();
			pstmt = conn.prepareStatement(DocInsuredPatAgreeSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocInsuredPatAgreeDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
				ConnectionManager.close(conn, pstmt, rs);
			} catch (SQLException e) {
