...
import com.digihealth.doc.sql.DocBadEventSql;
import com.digihealth.utils.ConnectionManager;

public class DocBadEventDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(DocBadEventSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------DocBadEventDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
