...
import com.digihealth.evt.sql.EvtMaterialSql;
import com.digihealth.utils.ConnectionManager;

public class EvtMaterialDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtMaterialSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtMaterialDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
