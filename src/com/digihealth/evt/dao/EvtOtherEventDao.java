...
import com.digihealth.evt.sql.EvtOtherEventSql;
import com.digihealth.utils.ConnectionManager;

public class EvtOtherEventDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtOtherEventSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtOtherEventDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
