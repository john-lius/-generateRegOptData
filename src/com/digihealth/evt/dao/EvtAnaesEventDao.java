...
import com.digihealth.evt.sql.EvtAnaesEventSql;
import com.digihealth.utils.ConnectionManager;

public class EvtAnaesEventDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtAnaesEventSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtAnaesEventDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
