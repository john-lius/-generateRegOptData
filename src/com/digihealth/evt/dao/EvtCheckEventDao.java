...
import com.digihealth.evt.sql.EvtCheckEventSql;
import com.digihealth.utils.ConnectionManager;

public class EvtCheckEventDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtCheckEventSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtCheckEventDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
