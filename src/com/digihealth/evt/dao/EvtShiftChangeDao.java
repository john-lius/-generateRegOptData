...
import com.digihealth.evt.sql.EvtShiftChangeSql;
import com.digihealth.utils.ConnectionManager;

public class EvtShiftChangeDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtShiftChangeSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtShiftChangeDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
