...
import com.digihealth.evt.sql.EvtInEventSql;
import com.digihealth.utils.ConnectionManager;

public class EvtInEventDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtInEventSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtInEventDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
