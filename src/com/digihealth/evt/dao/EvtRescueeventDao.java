...
import com.digihealth.evt.sql.EvtRescueeventSql;
import com.digihealth.utils.ConnectionManager;

public class EvtRescueeventDao {
	...
	public static void deleteByRegOptId(String name) {
		...
		try {
			...
			pstmt = conn.prepareStatement(EvtRescueeventSql.deleteByRegOptId);
			pstmt.setString(1, beid);
			pstmt.setString(2, "%" + name + "%");
			pstmt.executeUpdate();
		} catch (SQLException e) {
			System.out.println("----------EvtRescueeventDao-deleteByRegOptId----------" + e.getMessage());
		} finally {
			try {
